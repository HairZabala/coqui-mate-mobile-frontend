...
import { Alert, Linking, Platform, PlatformAndroidStatic, Rationale } from 'react-native';
...
export const defaultPhotoOptions: Options = {
  ...
  cropping: true,
  cropperActiveWidgetColor: theme.colors.primaryPurple,
};
...
const useImagePicker: (
  ...
  pickImage: (options?: Options) => Promise<ImagePickerResult | ImagePickerResult[] | undefined>;
  ...
  const requestPermission = useCallback(async (permission: Permission, rationale?: Rationale) => {
    try {
  ...
  const pickImage = async (options?: Options) => {
    try {
      ...
      if (permissionResult === Permissions.RESULTS.DENIED) {
        permissionResult = await requestPermission(mediaLibraryPermission, mediaPermssionRationale);
      }
      ...
      const pickerResponse = await ImageCropPicker.openPicker(options ?? defaultPhotoOptions);
  ...
  const takePhoto = async (options?: Options) => {
    try {
      ...
      if (permissionResult === Permissions.RESULTS.DENIED) {
        permissionResult = await requestPermission(cameraPermission, cameraPermissionRationale);
      }
      ...
      const pickerResponse = await ImageCropPicker.openCamera(options ?? defaultCameraOptions);
