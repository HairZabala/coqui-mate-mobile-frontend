...
import { SharedValue, useSharedValue, withRepeat, withSequence, withTiming } from 'react-native-reanimated';

const PULSE_DURATION = 2000;

const useLoadingAnimation: () => [SharedValue<number>, SharedValue<number>] = () => {
  ...
  useEffect(() => {
    ...
    opacityValue.value = withRepeat(
      withSequence(withTiming(0.5, { duration: PULSE_DURATION }), withTiming(1, { duration: PULSE_DURATION })),
      -1,
    );
